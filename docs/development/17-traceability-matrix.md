...
| REQ-007 | 工单处理、追加、关闭/重开和审核入库 | P0 | AC-006, AC-007 | `tickets`, `knowledge`, `documents`, `audit`, `web` | `backend/src/knowagent/tickets/application/workflow.py`, `backend/src/knowagent/tickets/application/review.py`, `backend/src/knowagent/tickets/domain/models.py`, `backend/src/knowagent/tickets/infrastructure/sqlalchemy_models.py`, `backend/src/knowagent/tickets/infrastructure/sqlalchemy_repository.py`, `backend/src/knowagent/tickets/ports.py`, `backend/migrations/versions/ee1a2b3c4d5e_add_ticket_workflow_and_review_tables.py`, `backend/tests/unit/test_ticket_workflow.py`, `backend/tests/unit/test_knowledge_review.py`, `backend/tests/unit/test_refusal_tickets.py`, `backend/tests/conftest.py` | implementing（工单状态机、回复追加、转换审计轨迹、知识候选审核与已发布知识片段已通过单测与静态检查；AC-007"5 分钟内可检索"依赖检索层纳入 TICKET 来源，工单 API 路由、迁移往返与真实链路验收待补） |
