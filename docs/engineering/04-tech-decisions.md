...
| TD-013 | 公司通知 API 适配与可靠投递 | 可配置 HTTP JSON Provider、固定协议后补适配器、任意脚本表达式 | 未知契约、事务一致性、安全、重试、后台可配置性 | 已确认：受控 JSON 模板 + Outbox |
...
### TD-013：可配置 HTTP JSON Provider + 事务 Outbox 通知

- **日期**：2026-08-08
- **决策**：公司通知采用项目自有 `NotificationProvider` 端口和 `httpx` HTTP JSON 适配器；管理员可配置启用状态、HTTPS 端点、鉴权 Header/密钥引用、受控 JSON 模板、成功状态码、超时和重试参数。工单事务内写入 Outbox，Celery 在事务提交后异步投递并持久化每次结果。
- **方案比较**：方案 A 可配置 HTTP JSON Provider 在真实协议未知时仍能用本地 Stub 完整验证，真实接口到位后通常只需调整受控模板；方案 B 固定内部协议更简单安全，但真实协议到位后必然二次开发；方案 C 允许任意脚本或请求表达式最灵活，但引入代码注入、密钥泄漏和沙箱维护风险。用户确认方案 A。
- **一致性与幂等**：创建工单通知系统负责人、负责人回复通知提问人；工单及回复与 Outbox 在同一数据库事务提交，外部调用失败不回滚业务状态。每个业务事件使用唯一幂等键，Worker 重复执行只能复用既有投递记录。
- **失败语义**：连接失败、超时、`408`、`429` 和 `5xx` 归类为临时失败，按配置指数退避；其他 `4xx`、模板/密钥配置错误归类为永久失败。达到最大次数后进入永久失败，管理员可查询并人工重试，人工重试会保留原始历史和审计记录。
- **安全**：数据库只保存密钥引用名，不保存或返回密钥明文；运行时从环境变量或公司密钥设施解析。生产环境要求 HTTPS，并通过环境配置的通知 Host 白名单限制管理员可配置目标，日志和回执摘要过滤鉴权 Header 与敏感正文。
- **模板边界**：模板是 JSON 值树，只允许替换白名单变量，不执行表达式、代码或任意属性访问。首版覆盖 `ticket_created` 与 `ticket_replied`，模板保存前执行字段、大小和占位符校验。
- **性能与成本**：复用现有 `httpx==0.28.1`、Celery、Redis 和 PostgreSQL，不新增依赖或服务；独立 `notification` 队列和批量恢复任务避免影响交互及文档任务。投递记录按分页和组合索引查询，不把 Provider 请求放入 API 事务。
- **验证与限制**：真实公司 API 尚未提供，先用 `httpx.MockTransport` 和本地 FastAPI Stub 覆盖成功、超时、限流、服务端错误、永久失败、幂等和人工重试；真实鉴权、限流、回执字段和类生产 Linux 集成仍是交付门禁。
- **回滚**：关闭通知配置即可停止新投递；Worker 关闭后 Outbox 与投递历史仍保存在 PostgreSQL。Provider 端口保持稳定，真实协议超出模板能力时可新增专用适配器而不改工单领域模型。
- **确认**：用户于 2026-08-08 确认方案 A，并授权新增通知配置、Outbox、投递记录表及迁移；密钥仅保存引用名。
